...
*Mechanics-only run-throughs of the engine (Fuel pools, Defend, binary armor, negative-HP wounds). No narration — just procedure, rolls, and the gauge. See [[Burners Adventure Game]].*
...
*Reminder — out of combat: `2d6 +` Approach vs Cost. Hostile magic: Fuel Defend vs cast total (see [[Burners Principles#Granular Defend, binary armor]]).*
...
- Marching order puts Brand on it. It springs, no telegraph, no Fuel up out of combat: Damage 2 beats Brand's AC 1, so the full **2 hits HP.** Brand 5 → 3.
...
*Note: each 5-HP rat took one blow to Crack and another to finish. A swarm's teeth are its count of attacks, not any one blow — each is Defended and tested against armor separately.*
...
Buried armed: **grave-sword** (2 slots) and a **grave-byrnie** (old chain, **AC 3**). HD 3 → **15 HP**. **Resistance 5 vs mundane arms** from grave-cold flesh, with silver the named weakness. AC and Resistance do not add; use the highest applicable threshold. The Referee announces it rising from its bier: **+2 Heat → 9.**

**Rolling Initiative.** The wight's pool is **14 dice** — 3 (HD) + 2 (sword) + 9 (Heat) — the hot scene fighting back hard. It reads six 3s/4s: **Initiative 6, first**. It strikes twice.

**Round 1.**

- **Wight**, two grave-sword blows:
  - → **Brand**: 3 dice (6,5,4 = 15). Brand has not meleed this round → one die: spends a 6 → 9 left; 9 beats leather AC 1, so **all 9 hit HP.** Brand 5 → **−4: a Wound** (severity 4). Location 1d6=2, **torso, winded** — loses a Fuel die. **Shock Check: 2d6 + Sword (0) = 8 vs severity 4** ✓ — keeps his feet. *(Brand Fuel 7→5.)*
  - → **Ilsa**: 2 dice (5,5 = 10). Ilsa has not meleed → one die: a 4 → 6 left; 6 beats leather AC 1, so **all 6 hit HP.** Ilsa HP 3 → **−3: a Wound** (severity 3). Location 1d6=4, an **arm** — the spear-arm hangs useless until she reaches full HP. **Shock Check: 2d6 + Sword (0) = 6 vs severity 3** ✓ — she keeps her feet, shaken, one-armed. *(Fuel 8→7.)*
...
- **Kragg** attacks the wight (Veteran, two blows; 2-handed axe, 4 slots → two dice a blow) — having meleed, he Defends without limit for the rest of this round:
  - Blow A: 6,5,4 = 15. Wight has already meleed this round, so Defends freely: a 6 → 9 left. 9 beats Resistance 5, so **all 9 hit.** Wight **15 → 6.**
  - Blow B: 5,4 = 9. Wight Defends a 5 → 4; Resistance 5 stops it. *Tink.* *(Wight Fuel 9→7.)*
- **Brand** attacks: sword, 1 die = 6. Wight Defends a 4 → 2; Resistance 5 stops it. *(Wight 7→6 dice.)*
...
- **Resistance vs the named weakness:** the wight's Resistance 5 made weak mundane blows go *tink!*, while committed blows got through in full and silver ignored the threshold (Sefa). Sorcery took the monster's arm out of the exchange so the fighters could finish. **Sorcerers solve; fighters kill.**
- **The monster's slot/Fuel economy cuts both ways:** the wight spent its huge pool attacking and Defending, so by Sefa's turn it could spare only a die or two. Tie up its dice — or its arm — then land the answer.
...
- **Ilsa** (her Action still unspent — Ward is not a turn): casts *Adhere* from her hand at the sharper. 1 die = **6** — cast total 6, and the 6 is a **Spark**. The card taps. The sharper, in the dirt and rattled, Defends with 2 dice: 3,2 = **5**. **Overrun 1** — his casting hand sticks to the floorboards (effect lane, **no HP**; he stays at 5). She spends the Spark on the working itself: the Referee lets the glue catch his other wrist as well — no ready cards until he washes free. *(Ilsa 5→4; sharper 6→4.)*
